// =============================================================================
//  reliability_test.ino
//  Project Sipat Banwa — Reliability Testing Firmware
//  Board  : ESP32
//  Purpose: 3–4 day soak test with full observability telemetry
//
//  Architecture (FreeRTOS dual-core):
//    Core 1 — sensorTask   : reads sensors every 1 second, logs to SD
//    Core 0 — uploadTask   : uploads to Firestore every 60 seconds
//    Core 0 — heartbeatTask: sends heartbeat to Firestore every 60 seconds
//
//  Sensors:
//    - Modbus RS485   : temperature + humidity   (SensorStatus OK/ERROR)
//    - INA219 (I2C)   : battery voltage, current, power, SoC
//    - Rainfall        : DISABLED (hardware not connected) — always 0.0 mm
//
//  Firebase: Firestore (HTTPS REST, PATCH, ?key=API_KEY)
//  SD:       CSV logging + pending-upload buffer
//
//  New reliability metrics (vs. initial version):
//    - boot_count          : persistent reboot counter (NVS/Preferences)
//    - min_heap            : heap low-water mark (memory leak detection)
//    - total_energy_Wh     : cumulative Wh consumed (battery sizing proof)
//    - peak_current_A      : session maximum current draw
//    - min_voltage         : session minimum battery voltage (brownout risk)
//    - internal_temp_c     : ESP32 die temperature (thermal check)
//    - dew_point           : derived from T+RH (Magnus formula)
//    - upload_latency_ms   : Firestore PATCH round-trip time
//    - wifi_reconnect_count: link drop + reconnect events
//    - consecutive_fail_streak / max_fail_streak : upload failure runs
//    - log_count           : total SD rows written
//    - sd_free_mb          : remaining SD card space
// =============================================================================
...
// Timing
const int   SENSOR_INTERVAL_MS  = 1000;   // 1 second
...
Preferences      prefs;       // NVS namespace for persistent boot_count
...
struct SensorReading {
  ...
  float     temperature;        // °C
  float     humidity;           // %
  float     heat_index;         // °C  (NOAA formula)
  float     dew_point;          // °C  (Magnus formula)  ← NEW

  // Rainfall — DISABLED, always 0.0
  float     rainfall_mm;
  ...
  float     total_energy_Wh;    // Wh — cumulative consumed   ← NEW
  float     peak_current_A;     // A  — session maximum       ← NEW
  float     min_voltage;        // V  — session minimum       ← NEW
  ...
  float     abs_humidity_gm3;           // g/m³ absolute humidity
  float     vpd_kPa;                    // kPa vapour pressure deficit
};
...
float    g_used_Ah         = 0.0;
float    g_total_energy_Wh = 0.0;
float    g_peak_current_A  = 0.0;
float    g_min_voltage     = 99.0;   // sentinel; set to first real reading
...
float heatIndex(float tempC, float rh) {
  if (tempC < 26.7 || rh < 40.0) return tempC;
...
// Valid for -40°C to +60°C, error < 0.35°C.
float dewPoint(float tempC, float rh) {
  if (rh <= 0.0f) return tempC;
...
// Accurate for 0–60°C, within ~0.3 g/m³.
float absoluteHumidity(float tempC, float rh) {
  float es = 6.1078f * exp(17.2694f * tempC / (tempC + 237.29f));  // hPa
  return (2165.0f * es * (rh / 100.0f)) / (273.15f + tempC);
}

// ── Vapour Pressure Deficit (kPa) ─────────────────────────────────────────────
// VPD = es × (1 – RH/100). Useful for enclosure / crop stress assessment.
float vaporPressureDeficit(float tempC, float rh) {
  float es = 0.6108f * exp(17.2694f * tempC / (tempC + 237.29f));  // kPa
...
// ── RTC Update from Serial ────────────────────────────────────────────────────
// Type "YYYY-MM-DD HH:MM:SS" in Serial Monitor to update the RTC clock.
void checkSerialRTCUpdate() {
  ...
  if (s.length() != 19 ||
    ...
    Serial.println("[RTC] Invalid format. Use: YYYY-MM-DD HH:MM:SS");
    return;
  }
  rtc.adjust(DateTime(
...
bool uploadReading(const SensorReading& r) {
  ...
  fsDouble(p, "heat_index",               r.heat_index);
  fsDouble(p, "dew_point",                r.dew_point);
  fsDouble(p, "abs_humidity_gm3",         r.abs_humidity_gm3);
  fsDouble(p, "vpd_kPa",                  r.vpd_kPa);
  ...
  fsDouble(p, "total_energy_Wh",          r.total_energy_Wh);
  fsDouble(p, "peak_current_A",           r.peak_current_A);
  fsDouble(p, "min_voltage",              r.min_voltage);
...
void sdWriteHeader(const char* path) {
  ...
  if (!f) return;
  f.println("timestamp,temperature,humidity,heat_index,dew_point,abs_humidity_gm3,vpd_kPa,"
            "rainfall_mm,rainfall_1h_mm,rain_raw,"
            "batt_voltage,batt_current_A,batt_power_W,batt_soc_pct,batt_remaining_Ah,"
            "total_energy_Wh,peak_current_A,min_voltage,"
...
bool sdAppendRow(const char* path, const SensorReading& r) {
  ...
  snprintf(row, sizeof(row),
    "%s,%.2f,%.2f,%.2f,%.2f,%.2f,%.3f,"       // env: ts, temp, hum, hi, dp, ah, vpd
    "%.2f,%.2f,%lu,"                         // rain: mm, 1h, raw
    "%.3f,%.4f,%.3f,%.1f,%.3f,"             // power: v, i, p, soc, ah
    "%.4f,%.4f,%.3f,"                        // power ext: wh, peak_i, min_v
    ...
    r.timestamp,
    r.temperature, r.humidity, r.heat_index, r.dew_point, r.abs_humidity_gm3, r.vpd_kPa,
    ...
    r.total_energy_Wh, r.peak_current_A, r.min_voltage,
...
void sensorTask(void* pvParams) {
  ...
  while (true) {
    ...
    float dp  = dewPoint(temperature, humidity);
    float ah  = absoluteHumidity(temperature, humidity);
    float vpd = vaporPressureDeficit(temperature, humidity);

    // ── Rainfall — DISABLED (hardware not connected) ──────────
    float    rainfall_mm    = 0.0f;
    float    rainfall_1h_mm = 0.0f;
    uint32_t rain_raw       = 0;
    ...
    // Coulomb counting + cumulative energy ← NEW
    g_used_Ah         += current_A * (SENSOR_INTERVAL_MS / 3600000.0f);
    g_total_energy_Wh += power_W   * (SENSOR_INTERVAL_MS / 3600000.0f);

    float remaining_Ah = CAPACITY_AH - g_used_Ah;
    float soc          = constrain((remaining_Ah / CAPACITY_AH) * 100.0f, 0.0f, 100.0f);

    // Peak / session-minimum tracking ← NEW
    if (current_A > g_peak_current_A) g_peak_current_A = current_A;
    if (voltage   < g_min_voltage)    g_min_voltage    = voltage;
    ...
    if (xSemaphoreTake(g_mutex, pdMS_TO_TICKS(500)) == pdTRUE) {
      ...
      g_reading.heat_index               = hi;
      g_reading.dew_point                = dp;
      ...
      g_reading.total_energy_Wh          = g_total_energy_Wh;
      g_reading.peak_current_A           = g_peak_current_A;
      g_reading.min_voltage              = g_min_voltage;
      ...
      g_reading.abs_humidity_gm3         = ah;
      g_reading.vpd_kPa                  = vpd;
      xSemaphoreGive(g_mutex);
    }

    // ── Log to SD (every second) ───────────────────────────────
    SensorReading snap;
    ...
    // ── Serial debug ───────────────────────────────────────────
    Serial.printf("[%s] T:%.1f H:%.1f HI:%.1f DP:%.1f AH:%.1f VPD:%.3f | "
                  "Batt:%.2fV %.3fA %.1f%% Wh:%.2f | "
                  "IntT:%.1f Heap:%lu(frag:%.0f%%) MB:%lums SD:%lums | %s\n",
                  ts,
                  temperature, humidity, hi, dp, ah, vpd,
                  voltage, current_A, soc, g_total_energy_Wh,
                  int_temp, (unsigned long)heap, heapFrag,
                  (unsigned long)readLatency, (unsigned long)g_sd_write_latency_ms,
                  sensorStatus.c_str());
...
void setup() {
  ...
  // ── INA219 ─────────────────────────────────────────────────
  if (!ina219.begin()) {
    Serial.println("[INA219] ERROR: not detected. Check wiring.");
  } else {
    ina219.setCalibration_32V_2A();
    Serial.println("[INA219] OK — calibrated at 32V/2A");
    Serial.printf("[INA219] Placement: 12V battery → INA219 → Step-Down → Load\n");
  }
  ...
  g_reading.reset_reason  = g_reset_reason;
  g_reading.boot_count    = g_boot_count;
  g_min_voltage           = 99.0f;   // sentinel before first INA219 read
