...
describe("API smoke tests", () => {
  ...
  test("GET /api/api/auth/session works when proxy duplicates /api prefix", async () => {
    const res = await fetchApp(new Request("http://localhost/api/api/auth/session"));
    expect(res.status).toBe(200);
  ...
  test("GET /proxy/api/latest works when app is mounted under a prefix", async () => {
    const res = await fetchApp(new Request("http://localhost/proxy/api/latest"));
    expect(res.status).toBe(200);
  });
